...
- [fabric-loop-pointer](./fabric-loop-pointer/SKILL.md) — 转发指针：裸 home lane 说「走 fabric loop / 织锦审核 / 生态体检」时把会话导去织锦 repo 内的项目作用域真 skill（L1/L2 分层审核 machinery），本体无独立逻辑。Source: 2026-07 建于 ~/.claude/skills 孤本，2026-09-01 迁入 master
- [fabric-architect-pointer](./fabric-architect-pointer/SKILL.md) — 转发指针：裸 home lane 问织锦架构类问题（怎么设计的/影响面多大/有哪些域表端点/归谁管）时装载织锦 repo 内 fabric-architect 只读咨询 agent 的配方经 general-purpose 委派（tools 白名单降级为 prompt 约束，结果须注明降级通道）。Source: session-derived 2026-09-01
