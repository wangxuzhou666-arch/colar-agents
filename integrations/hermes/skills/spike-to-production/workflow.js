...
5. 体积压缩方案要**按三类资产分开给**，它们的消费路径不同、压法不能互相代替
   （详见 SKILL.md Step 3，这里只列判据）：
   · 平铺贴图 → 降分辨率（normal/rough/ao 降 1K，albedo 保 2K）
   · 列表缩略图 → 转 WebP。这是**格式选错**不是分辨率问题；瓶颈常在请求数而非字节数
     （单域名并发 6，几十张要排好几轮）。优先让**导入脚本直接落 WebP**，别只给事后批处理。
   · GLB → 先量贴图占字节的比例（多半 70%+），走 KTX2 纹理 + meshopt 几何。
     **gzip 对已嵌入的 PNG 完全无效**，别把"nginx 开了 gzip"当已压缩。
     必须同时确认：产品侧 loader 接没接 KTX2Loader + MeshoptDecoder（不接是整件载入失败，
     不是画质降级）、transcoder wasm 有没有落到 public/、材质名是不是槽位映射的 key
     （是的话压缩链必须保材质名，gltfpack 要带 -km -kn）。
   · 环境贴图 / HDRI → 降**宽度**，判据不是体积。three 的 PMREM 对等距柱状图走
     _setSize(width/4)，cube = 2^floor(log2(宽/4))（2048→512、1024→256、512→128），
     所以降宽度 = 降反射 mip 链的基底分辨率，直接影响低粗糙度表面的高光锐度。
     缩放必须用盒式平均（INTER_AREA）保能量，校验读**回来**那份（写盘要过 RGBE 量化），
     看平均亮度漂移与峰值两个数。定档靠烘封面并排比高光，别把 GLB 那条 45.8dB 线套过来
     （块状伪影 vs 平滑高光位移，同 PSNR 下后者宽容得多）。
6. 查产品仓有没有**机械门**兜住"忘了跑压缩"：进 git 的资产该拦在 pre-commit，
   不进 git 的（只走部署 rsync）该拦在部署预检。没有就把它列进移植方案。
   **门本身要可测且验证过能变红**：驱动收进 main()（模块级驱动 import 即执行，没法单测），
   判不出来时出 warning 而不是静默放行，并跑 mutation 确认能红。
   另外，压缩脚本的中间产物与备份**不能落进被 rsync 的资产目录**（rsync 不读 .gitignore，
   原件会跟着上生产机把收益抵消）——写 /tmp 再 mv，并在脚本里加守卫拒绝该目录。

输出：lane 结论（照抄哪条现有 convention）+ 需要新补的 .gitignore 行
+ 按上述四类分开的体积压缩方案 + 压缩门的落点建议（含门的可测性）。`,
...
**另外必答一问（本次栽过）**：spike 侧有没有**针对某个具体资产**的常量/覆盖表
（"这件的明线材质叫 X"、"这件的挂钩要隐藏"）？有的话，逐条查它针对的那件资产
**在目标仓的库里吗**。不在 = 规则搬过去也匹配不到任何东西，等于没搬，
而且看起来像搬过了。把"要一并搬的资产"列进输出。
...
5. 部署脚本自己的 fail-closed 门（如"工作树干净门"）会不会拦？先量：git status --short
   看脏在哪，再 grep sync.sh 的 --exclude 看那些文件是否**根本不会被同步上去**。
   若脏文件全落在被 exclude 的目录里，说明门的判据比实际风险面宽 —— 把这个证据写进输出，
   连同 ALLOW_DIRTY 之类的绕行开关一起**交给 Colar 拍板**，不要自己决定绕，也不要因此放弃部署。
${a.deployHost ? `6. 部署档：${a.deployHost}，读 scripts/deploy/hosts/ 确认目标机与域名。` : ''}
